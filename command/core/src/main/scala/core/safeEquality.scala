...
object safeEquality extends cats.syntax.EqSyntax {
  ...
  implicit val doubleEq: Eq[Double] = instances.double.catsKernelStdOrderForDouble
  implicit val bigDecimalEq: Eq[BigDecimal] = instances.bigDecimal.catsKernelStdOrderForBigDecimal
  implicit val stringEq: Eq[String] = instances.string.catsKernelStdOrderForString
  implicit val charEq: Eq[Char] = instances.char.catsKernelStdOrderForChar
  implicit val booleanEq: Eq[Boolean] = instances.boolean.catsKernelStdOrderForBoolean
  implicit def listEq[A: Order]: Eq[List[A]] = instances.list.catsKernelStdOrderForList
  implicit def optionEq[A: Order]: Eq[Option[A]] = instances.option.catsKernelStdOrderForOption
  implicit def setEq[A]: Eq[Set[A]] = instances.set.catsKernelStdPartialOrderForSet
  implicit def mapEq[K, V: Eq]: Eq[Map[K, V]] = instances.map.catsKernelStdEqForMap
